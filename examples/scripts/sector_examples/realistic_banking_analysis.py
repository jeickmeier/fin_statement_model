...
def validate_banking_data(raw_data: dict[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
...
def perform_stress_testing(graph: Graph, metrics: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for scenario_name, scenario_params in scenarios.items():
        ...
        if all(
            node in data_nodes
            for node in ["gross_loans", "total_deposits", "securities_available_for_sale"]
        ):
    ...
    for title, category, ax in camels_components:
        if category in metrics and periods[-1] in metrics[category]:
            ...
            if metric_name:
                ...
                if valid_values:
                    ax.plot(valid_periods, valid_values, marker="o", linewidth=2, markersize=8)
