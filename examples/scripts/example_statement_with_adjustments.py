...
from fin_statement_model.core.adjustments.models import AdjustmentType # Added AdjustmentType
from fin_statement_model.core.nodes import FormulaCalculationNode # Added FormulaCalculationNode
...
forecast_configs = {
    ...
    "core.accounts_payable": {"method": "curve", "config": [0.04, 0.03, 0.03, 0.02, 0.02]},
    ...
    "core.opex": {"method": "statistical", "config": {"distribution": "normal", "params": {"mean": 0.03, "std": 0.015}}},
}
...
try:
    ...
    if revenue_node and cogs_node:
        # Assuming the built-in gross_profit metric uses formula 'revenue - cost_of_goods_sold'
        # We map these formula variables to the actual nodes in the graph.
        gp_node = FormulaCalculationNode(
            name="gross_profit",
            inputs={"revenue": revenue_node, "cost_of_goods_sold": cogs_node},
            formula="revenue - cost_of_goods_sold",
            metric_name="gross_profit" # Optional: link to original metric
        )
        graph.add_node(gp_node)
        logger.info("Manually added 'gross_profit' FormulaCalculationNode to the graph.")
    else:
        logger.warning("Skipping manual gross_profit node addition: Input nodes (core.revenue, core.cogs) not found.")
