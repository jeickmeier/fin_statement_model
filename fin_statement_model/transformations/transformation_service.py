...
class TransformationService:
    ...
    def _register_builtin_transformers(self):
        ...
        if 'normalization' not in registered_transformers:
            TransformerFactory.register_transformer('normalization', NormalizationTransformer)
        
        if 'time_series' not in registered_transformers:
            TransformerFactory.register_transformer('time_series', TimeSeriesTransformer)
        
        if 'period_conversion' not in registered_transformers:
            TransformerFactory.register_transformer('period_conversion', PeriodConversionTransformer)
        
        if 'statement_formatting' not in registered_transformers:
            TransformerFactory.register_transformer('statement_formatting', StatementFormattingTransformer)
        
    def normalize_data(self, data: Union[pd.DataFrame, Dict], 
                      normalization_type: str = 'percent_of',
                      reference: Optional[str] = None,
                      scale_factor: Optional[float] = None) -> Union[pd.DataFrame, Dict]:
        """
    ...
    def transform_time_series(self, data: Union[pd.DataFrame, Dict],
                            transformation_type: str = 'growth_rate',
                            periods: int = 1,
                            window_size: int = 3) -> Union[pd.DataFrame, Dict]:
        """
    ...
    def convert_periods(self, data: pd.DataFrame,
                      conversion_type: str,
    ...
    def format_statement(self, data: pd.DataFrame,
                       statement_type: str = 'income_statement',
                       add_subtotals: bool = True,
                       apply_sign_convention: bool = True) -> pd.DataFrame:
        """
    ...
    def apply_transformation_pipeline(self, data: Any, transformers_config: List[Dict[str, Any]]) -> Any:
        """
