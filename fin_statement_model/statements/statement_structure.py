    ...
    def __init__(
        ...
        if not id or not isinstance(id, str):
            raise StatementError(
                message=f"Invalid line item ID: {id}"
            )
...
class CalculatedLineItem(LineItem):
    ...
    def __init__(
        ...
        if "type" not in calculation:
            raise StatementError(
                message=f"Missing calculation type for item: {id}"
            )
        
        if "inputs" not in calculation:
            raise StatementError(
                message=f"Missing calculation inputs for item: {id}"
            )
...
class Section:
    ...
    def __init__(
        ...
        if not id or not isinstance(id, str):
            raise StatementError(
                message=f"Invalid section ID: {id}"
            )
...
class StatementStructure:
    ...
    def __init__(
        ...
        if not id or not isinstance(id, str):
            raise StatementError(
                message=f"Invalid statement ID: {id}"
            )
    ...
    def get_items_by_type(self, item_type: StatementItemType) -> List[Union[Section, StatementItem]]:
        """
