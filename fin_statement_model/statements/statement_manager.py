...
class StatementManager:
    ...
    def load_statement(self, config_path: str) -> StatementStructure:
        ...
        try:
            ...
            logger.error(f"Unexpected error loading statement configuration from {config_path}: {e}")
    ...
    def create_calculations(self, statement_id: str) -> List[str]:
        ...
        # Keep track of items in the current dependency chain to detect circular dependencies
        ...
        try:
            ...
            while calc_items and len(processed) < len(calc_items):
                ...
                if not progress_made:
                    ...
                    logger.error(f"Circular dependency detected in statement '{statement_id}': {cycle}")
            ...
            logger.info(f"Created {len(created_nodes)} calculation nodes for statement '{statement_id}'")
            ...
            logger.error(f"Error creating calculations for statement '{statement_id}': {e}")
                ...
                if self.graph.get_node(node_id) is not None:
                    ...
                    for period in self.graph.periods:
                        try:
                            ...
                            logger.warning(f"Error calculating {node_id} for {period}: {e}")
    ...
    def format_statement(self, statement_id: str, format_type: str = 'dataframe', **kwargs) -> Any:
        """
    ...
    def export_to_excel(self, statement_id: str, file_path: str, **kwargs) -> None:
        ...
        try:
            ...
            if isinstance(e, StatementError):
                ...
                # Wrap the exception
                logger.error(f"Error exporting statement '{statement_id}' to {file_path}: {e}")
                raise ExportError(
