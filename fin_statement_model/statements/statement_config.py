...
from ..core.errors import ConfigurationError, StatementError
...
class StatementConfig:
    ...
    def load_config(self, config_path: str) -> None:
        ...
        try:
            ...
            if hasattr(e, 'problem_mark'):
                mark = e.problem_mark
                error_detail = f"YAML parse error at line {mark.line+1}, column {mark.column+1}: {e.problem}"
            else:
                error_detail = str(e)
            raise ConfigurationError(
    ...
    def _validate_item(self, item: Dict[str, Any], section_idx: int, item_idx: int) -> List[str]:
        ...
        for field in required_fields:
            if field not in item:
                errors.append(f"Section[{section_idx}].Item[{item_idx}]: Missing required field: {field}")
        ...
        if not item_id:
            ...
            errors.append(f"Section[{section_idx}].Item[{item_idx}]: ID '{item_id}' should not contain spaces")
        ...
        if item_type == 'section':
            ...
            if 'items' not in item:
                errors.append(f"Section[{section_idx}].Item[{item_idx}]: Nested section missing 'items' field")
            else:
                nested_items = item.get('items', [])
                if not isinstance(nested_items, list):
                    errors.append(f"Section[{section_idx}].Item[{item_idx}]: 'items' must be a list")
                else:
                    for k, nested_item in enumerate(nested_items):
                        nested_errors = self._validate_item(nested_item, f"{section_idx}.{item_idx}", k)
                        errors.extend(nested_errors)
        
        elif item_type == 'line_item':
            # Basic line item should have a node_id
            if 'node_id' not in item:
                errors.append(f"Section[{section_idx}].Item[{item_idx}]: Line item missing 'node_id' field")
        
        elif item_type == 'calculated':
            if 'calculation' not in item:
                errors.append(f"Section[{section_idx}].Item[{item_idx}]: Calculated item missing 'calculation' field")
            else:
                calculation = item.get('calculation', {})
                if not isinstance(calculation, dict):
                    errors.append(f"Section[{section_idx}].Item[{item_idx}]: 'calculation' must be a dictionary")
                else:
                    # Validate calculation
                    if 'type' not in calculation:
                        errors.append(f"Section[{section_idx}].Item[{item_idx}]: Calculation missing 'type' field")
                    if 'inputs' not in calculation:
                        errors.append(f"Section[{section_idx}].Item[{item_idx}]: Calculation missing 'inputs' field")
                    elif not isinstance(calculation.get('inputs', []), list):
                        errors.append(f"Section[{section_idx}].Item[{item_idx}]: 'inputs' must be a list")
            ...
            if 'calculation' in item:
                ...
                if calculation.get('type') != 'addition':
                    errors.append(f"Section[{section_idx}].Item[{item_idx}]: Subtotal calculation type must be 'addition'")
            elif 'items_to_sum' not in item:
                errors.append(f"Section[{section_idx}].Item[{item_idx}]: Subtotal item missing 'items_to_sum' field")
            elif not isinstance(item.get('items_to_sum', []), list):
                errors.append(f"Section[{section_idx}].Item[{item_idx}]: 'items_to_sum' must be a list")
        
        else:
            errors.append(f"Section[{section_idx}].Item[{item_idx}]: Unknown item type: {item_type}")
    ...
    def build_statement_structure(self) -> StatementStructure:
        ...
        try:
            ...
            logger.error(f"Error building statement structure: {e}")
            raise ConfigurationError(
    ...
    def _build_section(self, section_config: Dict[str, Any]) -> Section:
        ...
        try:
            ...
            section_id = section_config.get('id', 'unknown')
            raise ConfigurationError(
                message=f"Failed to build section",
                errors=[f"Error in section '{section_id}': {str(e)}"]
            ) from e
    
    def _build_item(self, item_config: Dict[str, Any]) -> Union[LineItem, CalculatedLineItem, SubtotalLineItem, Section]:
        """
        ...
        try:
            ...
            if item_type == 'section':
                ...
                raise ConfigurationError(
                    ...
                    errors=[f"Item '{item_config.get('id', 'unknown')}' has invalid type: {item_type}"]
                )
        except KeyError as e:
            ...
            item_id = item_config.get('id', 'unknown')
            raise ConfigurationError(
                message=f"Failed to build item",
                errors=[f"Error in item '{item_id}': {str(e)}"]
            ) from e
...
def load_statement_config(config_path: str) -> StatementStructure:
    ...
    try:
        ...
        if isinstance(e, ConfigurationError):
            ...
            logger.error(f"Unexpected error loading statement configuration: {e}")
            raise ConfigurationError(
