...
class StatementConfig:
    ...
    def _validate_section(self, section: Dict[str, Any], index: int) -> List[str]:
        ...
        if not section_id:
            ...
            errors.append(
                f"Section[{index}]: ID '{section_id}' should not contain spaces"
            )
        ...
        if "subsections" in section:
            ...
            if not isinstance(subsections, list):
                ...
                for j, subsection in enumerate(subsections):
                    subsection_errors = self._validate_section(
                        subsection, f"{index}.{j}"
                    )
    ...
    def _validate_item(
        self, item: Dict[str, Any], section_idx: int, item_idx: int
    ) -> List[str]:
        """
        ...
        if not item_id:
            errors.append(
                f"Section[{section_idx}].Item[{item_idx}]: ID cannot be empty"
            )
        elif not isinstance(item_id, str):
            errors.append(
                f"Section[{section_idx}].Item[{item_idx}]: ID must be a string"
            )
        elif " " in item_id:
            errors.append(
    ...
    def build_statement_structure(self) -> StatementStructure:
        ...
        if errors:
            ...
            raise ConfigurationError(
                message=error_msg, config_path=self.config_path, errors=errors
            )
    ...
    def _build_section(self, section_config: Dict[str, Any]) -> Section:
        ...
        try:
            ...
            raise ConfigurationError(
                ...
                errors=[
                    f"Section '{section_id}' is missing required field: {e.args[0]}"
                ],
            ) from e
        except Exception as e:
            section_id = section_config.get("id", "unknown")
    ...
    def _build_subtotal(self, subtotal_config: Dict[str, Any]) -> SubtotalLineItem:
        ...
        try:
            ...
            raise ConfigurationError(
                message=f"Missing required field '{e.args[0]}'",
                errors=[
                    f"Subtotal '{subtotal_id}' is missing required field: {e.args[0]}"
                ],
            ) from e
        except Exception as e:
            subtotal_id = subtotal_config.get("id", "unknown")
