...
from .errors import NodeError, GraphError, CalculationError, CircularDependencyError, PeriodError

# Use TYPE_CHECKING to avoid circular imports
if TYPE_CHECKING:
    from .engine import CalculationEngine  # pragma: no cover
...
class Graph:
    ...
    def set_calculation_engine(self, engine):
        ...
        from .engine import CalculationEngine  # Import here to avoid circular import
    ...
    def import_data(self, data: Dict[str, Dict[str, float]]) -> None:
        ...
        for node_id, period_values in data.items():
            if not isinstance(period_values, dict):
                raise ValueError(f"Invalid data format for node '{node_id}': expected dict, got {type(period_values)}")
                
            # Validate values are numbers
            for period, value in period_values.items():
                if not isinstance(value, (int, float)):
                    raise ValueError(f"Invalid value for node '{node_id}' period '{period}': expected number, got {type(value)}")
            ...
            if hasattr(node, 'inputs'):
                for inp in node.inputs:
                    if not self.has_node(inp.name):
                        errors.append(f"Node '{node_id}' depends on non-existent node '{inp.name}'")
    ...
    def remove_node(self, id: str) -> None:
        ...
        if id not in self.nodes:
            raise NodeError(
                message=f"Node '{id}' does not exist",
                node_id=id
            )
