...
class FinancialStatementGraph:
    ...
    def __init__(self, periods: Optional[List[str]] = None):
        ...
        logger.info(
            f"FinancialStatementGraph initialized with {len(periods or [])} periods"
        )
    ...
    def add_calculation(
        self, name: str, inputs: List[str], calculation_type: str
    ) -> str:
        """
    ...
    def calculate_financial_statement(
        self, node_name: Optional[str] = None, period: Optional[str] = None
    ) -> Dict:
        """
    ...
    def create_forecast(
        ...
        growth_rates: Optional[
            Dict[str, Union[float, List[float], Dict[str, Any]]]
        ] = None,
        method: Union[str, Dict[str, str]] = "simple",
        **kwargs,
    ) -> None:
        """
        ...
        try:
            ...
            for node_name, growth_rate in growth_rates.items():
                ...
                if node_method == "simple":
                    ...
                    if (
                        not isinstance(growth_rate, dict)
                        or "distribution" not in growth_rate
                    ):
                        raise ValueError(
                ...
                self._forecast_node(
                    node, historical_periods, forecast_periods, growth_rate, node_method
                )
    ...
    def _forecast_node(
        self, node, historical_periods, forecast_periods, growth_rate, method, **kwargs
    ):
        """Forecast a node using the specified method."""
        # Get the last historical period to use as base
        base_period = historical_periods[-1]  # type: ignore
        ...
        if method == "simple":
            ...
            if len(growth_rate) != len(forecast_periods):
                raise ValueError(
                    f"Growth rates list for {node.name} must match the number of forecast periods"
                )

            growth_params = [
                float(rate) for rate in growth_rate
            ...
            distribution = growth_rate["distribution"]  # type: ignore
            params = growth_rate["params"]  # type: ignore
            ...
            raise ValueError(
                f"Invalid forecasting method: {method}"
            )  # pragma: no cover
    ...
    def import_from_dataframe(
        ...
        try:
            ...
            imported_graph = self._importer.import_from_dataframe(
                df=df, statement_type=statement_type, mapping_config=mapping_config
            )
    ...
    def to_excel(
        self, file_path: Union[str, Path], sheet_name: str = "Financial Statement"
    ) -> None:
        """
    ...
    def convert_periods(
        self, conversion_type: str, aggregation: str = "sum"
    ) -> pd.DataFrame:
        """
        ...
        """
        df = self.to_dataframe()
        return self._transformation_service.convert_periods(
            df, conversion_type, aggregation
        )
    ...
    def apply_transformations(
        ...
        """
        df = self.to_dataframe()
        return self._transformation_service.apply_transformation_pipeline(
            df, transformers_config
        )
        ...
        for node in self.get_financial_statement_items():
            for period in node.values:
                if (
                    ...
                    and period not in historical_periods
                    and node.values[period] != 0.0
                ):  # type: ignore # Assuming zero values might be placeholders
    ...
    def __str__(self) -> str:
        """
        Return a summary of the financial statement graph including nodes, calculations,
        metrics, periods, and other key information.

        This method provides a comprehensive overview of the graph structure and data
        for debugging and analysis purposes.

        Returns:
            str: A formatted string containing the graph summary
        """
        from .nodes import CalculationNode
        ...
        all_periods = list(self.graph._periods)
        historical_periods = self.get_historical_periods()
        forecast_periods = [p for p in all_periods if p not in historical_periods]

        summary.append(f"PERIODS ({len(all_periods)}):")
        summary.append(
            f"  Historical ({len(historical_periods)}): {', '.join(historical_periods)}"
        )
        summary.append(f"  Forecast ({len(forecast_periods)}): {', '.join(forecast_periods)}")

        # Get nodes by type
        fs_items = self.get_financial_statement_items()
        calc_nodes = [
            node
            for node in self.graph.nodes.values()
        ...
        summary.append(f"  Financial Statement Items: {len(fs_items)}")
        summary.append(f"  Calculations: {len(calc_nodes)}")

        # Add financial statement items with sample values
        if fs_items:
            summary.append("\nFINANCIAL STATEMENT ITEMS:")
            for node in sorted(fs_items, key=lambda x: x.name):
                sample_period = next(iter(node.values)) if node.values else None
                sample_value = (
                    node.values.get(sample_period, "N/A") if sample_period else "N/A"
                )
                summary.append(
                    f"  {node.name}: {sample_value} ({sample_period if sample_period else 'No data'})"
                )

        # Add calculations with their inputs and strategy
        if calc_nodes:
            summary.append("\nCALCULATIONS:")
            for node in sorted(calc_nodes, key=lambda x: x.name):
                inputs = [
                    self.graph.nodes[input_id].name
                    for input_id in node.inputs
                    if input_id in self.graph.nodes
                ]
                strategy = node.__class__.__name__.replace('Node', '').lower()
                summary.append(f"  {node.name}: {strategy} of {inputs}")
    ...
    def __add__(self, other: "FinancialStatementGraph") -> "FinancialStatementGraph":
        """
        Merge two financial statement graphs together.

        Args:
            other: Another FinancialStatementGraph to merge with this one

        Returns:
            A new FinancialStatementGraph containing nodes from both graphs
        ...
        # Create new graph with combined periods
        combined_periods = sorted(list(set(self.graph._periods) | set(other.graph._periods)))
        merged = FinancialStatementGraph(periods=combined_periods)

        # Helper to merge node values
        def merge_values(node1_values: dict, node2_values: dict, node_name: str) -> dict:
            merged_values = node1_values.copy()
            for period, value in node2_values.items():
                if period in merged_values and merged_values[period] != value:
                    raise ValueError(
                        f"Conflicting values for node '{node_name}' in period {period}: "
                        f"{merged_values[period]} != {value}"
                    )
                merged_values[period] = value
            return merged_values

        # Merge financial statement items
        all_fs_items = set(self.get_financial_statement_items()) | set(
            other.get_financial_statement_items()
        )
        for item in all_fs_items:
            node1 = self.graph.nodes.get(item.name)
            node2 = other.graph.nodes.get(item.name)
            
            if node1 and node2:
                # Node exists in both graphs - merge values
                merged_values = merge_values(node1.values, node2.values, item.name)
                merged._data_manager.add_item(item.name, merged_values)
            elif node1:
                # Node only in first graph
                merged._data_manager.add_item(item.name, node1.values)
            else:
                # Node only in second graph
                merged._data_manager.add_item(item.name, node2.values)

        # Merge calculation nodes
        all_calc_nodes = [
            node for node in (set(self.graph.nodes.values()) | set(other.graph.nodes.values()))
            if isinstance(node, CalculationNode)
        ]
        for calc_node in all_calc_nodes:
            if calc_node.name not in merged.graph.nodes:
                merged.graph.add_node(calc_node)
