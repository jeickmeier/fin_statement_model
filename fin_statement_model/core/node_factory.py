...
class NodeFactory:
    ...
    def create_calculation_node(
        cls, name: str, inputs: List[Node], calculation_type: str, **kwargs
    ) -> CalculationNode:
        """
        ...
        if use_legacy and calculation_type in cls._calculation_node_types:
            ...
            logger.debug(
                f"Creating legacy calculation node of type '{calculation_type}': {name}"
            )
        ...
        logger.debug(
            f"Creating strategy calculation node with '{strategy_name}' strategy: {name}"
        )
    ...
    def create_forecast_node(
        ...
        forecast_type: str,
        growth_params: Union[float, List[float], Callable],
    ) -> ForecastNode:
        """
        ...
        if forecast_type not in cls._forecast_node_types:
            ...
            raise ValueError(
                f"Invalid forecast type: {forecast_type}. Valid types are: {valid_types}"
            )
        ...
        logger.debug(
            f"Creating forecast node of type '{forecast_type}' for base node: {base_node.name}"
        )
    ...
    def create_metric_node(
        cls, name: str, inputs: List[Node], formula: Callable, description: str = None
    ) -> MetricCalculationNode:
        """
    ...
    @classmethod
    def _validate_growth_params(
        cls, forecast_type: str, growth_params: Any, forecast_periods: List[str]
    ) -> None:
        """
        ...
        if forecast_type == "fixed":
            if not isinstance(growth_params, (int, float)):
                raise ValueError(
                    "For fixed growth forecast, growth_params must be a number"
                )

        elif forecast_type == "curve":
            if not isinstance(growth_params, list) or len(growth_params) != len(
                forecast_periods
            ):
                raise ValueError(
                    "For curve growth forecast, growth_params must be a list with one value per forecast period"
                )

        elif forecast_type == "statistical" or forecast_type == "custom":
            if not callable(growth_params):
                raise ValueError(
                    f"For {forecast_type} growth forecast, growth_params must be callable"
                )  # pragma: no cover
