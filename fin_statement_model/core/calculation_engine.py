...
class CalculationEngine:
    ...
    def add_calculation(
        self, name: str, input_names: List[str], operation_type: str, **kwargs
    ) -> Node:
        """
        ...
        node = NodeFactory.create_calculation_node(
            name, input_nodes, operation_type, **kwargs
        )
        ...
        logger.debug(
            f"Added calculation node '{name}' with operation '{operation_type}' to the graph"
        )
        return node
    ...
    def add_metric(self, metric_name: str, node_name: str = None) -> Node:
            ...
            if "inputs" in metric_def:
                ...
                for input_name in inputs:
                    if self.graph.get_node(input_name) is None:
                        raise ValueError(
                            f"Required input '{input_name}' for metric '{metric_name}' not found in graph."
                        )
                ...
                def formula_func(*args):
                    ...
                    values_dict = {
                        input_name: value for input_name, value in zip(inputs, args)
                    }
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Error evaluating formula '{formula_str}': {e}"
                        )
        ...
        # If it's a derived metric that needs its own function
            ...
            raise NotImplementedError(
                f"Custom metric function for '{metric_name}' not yet implemented."
            )

        else:
            raise ValueError(
                f"Metric '{metric_name}' definition is invalid - missing formula or function."
            )
    ...
    def change_calculation_strategy(
        self, node_name: str, new_strategy_name: str, **kwargs
    ) -> None:
        """
        ...
        new_strategy = CalculationStrategyRegistry.get_strategy(
            new_strategy_name, **kwargs
        )

        # Change the strategy
        node.set_strategy(new_strategy)
        logger.debug(
            f"Changed calculation strategy for node '{node_name}' to '{new_strategy_name}'"
        )
