...
from .errors import CalculationError, NodeError, CircularDependencyError, StrategyError
from ..calculations import CalculationStrategy, CalculationStrategyRegistry
...
class CalculationEngine:
    ...
    def add_calculation(
        self, node_id: str, input_nodes: List[str], calc_type: str, **params
    ) -> str:
        """
        ...
        """
        if not self._graph:
            raise CalculationError(message="No graph assigned to calculation engine")
        ...
        try:
            ...
            input_node_objects = [
                self._graph.get_node(input_id) for input_id in input_nodes
            ]
            ...
            if isinstance(e, (StrategyError, NodeError)):
                ...
                raise CalculationError(
                    message="Failed to add calculation node",
                    node_id=node_id,
    ...
    def calculate(self, node_id: str, period: str) -> float:
        ...
        if not node:
            raise NodeError(message=f"Node not found: {node_id}", node_id=node_id)
    ...
    def _execute_calculation(self, node, period: str) -> float:
        ...
        try:
            ...
            if isinstance(
                e, (CalculationError, NodeError, CircularDependencyError, StrategyError)
            ):
                # Re-raise specific errors
                raise
            else:
    ...
    def register_strategy(
        self, strategy_type: str, strategy: CalculationStrategy
    ) -> None:
        """
        ...
        try:
            ...
            raise StrategyError(
                message="Failed to register calculation strategy",
    ...
    def get_strategy(self, strategy_type: str) -> Optional[CalculationStrategy]:
        ...
        try:
            ...
            raise StrategyError(
                message="Failed to get calculation strategy",
                strategy_type=strategy_type,
            ) from e
