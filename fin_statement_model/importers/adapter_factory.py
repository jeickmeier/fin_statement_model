...
class AdapterFactory:
    ...
    def register_adapter(
        cls, adapter_type: str, adapter_class: Type[DataSourceAdapter]
    ) -> None:
        """
        ...
        Returns:
            None: Returns silently if the same adapter class is already registered
        """
        if adapter_type in cls._adapters:
            # Check if we're registering the exact same class
            if cls._adapters[adapter_type] == adapter_class:
                # Same class being registered, just return silently
                logger.debug(
                    f"Adapter type '{adapter_type}' is already registered with the same class."
                )
                return
            else:
                # Different class being registered, log warning and overwrite
                logger.warning(
                    f"Adapter type '{adapter_type}' is being overwritten with a different class."
                )
    ...
    def create_adapter(cls, adapter_type: str, **kwargs) -> DataSourceAdapter:
        ...
        if adapter_type not in cls._adapters:
            raise ValueError(
                f"Adapter type '{adapter_type}' is not registered. Available types: {list(cls._adapters.keys())}"
            )
    ...
    def discover_adapters(
        cls, package_path: str = "fin_statement_model.importers"
    ) -> None:
        """
            ...
            for _, module_name, is_pkg in pkgutil.iter_modules(package_dir):
                try:
                    ...
                    for name, obj in module.__dict__.items():
                        if (
                            isinstance(obj, type)
                            and issubclass(obj, DataSourceAdapter)
                            and obj
                            not in [
                                DataSourceAdapter,
                    ...
                    logger.error(
                        f"Error discovering adapters in module {module_name}: {e}"
                    )
