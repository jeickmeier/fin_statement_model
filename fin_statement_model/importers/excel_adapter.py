...
class ExcelAdapter(FileDataSourceAdapter):
    ...
    def _init_field_mappings(self, mapping_config: Optional[Dict] = None) -> None:
        ...
        if mapping_config:
            if 'income_statement' in mapping_config:
                self.income_statement_field_mapping.update(mapping_config['income_statement'])
    ...
    def extract_time_periods(self, file_path: str, sheet_name: str, 
                           period_column: str) -> List[str]:
        """
    ...
    def fetch_statement(self, file_path: str, sheet_name: str, 
                      period_column: str, statement_type: str = 'income_statement') -> Dict:
        """
    ...
    def create_statement_graph(self, file_path: str, sheet_name: str, 
                             period_column: str, 
        ...
        try:
            # Fetch raw data
            data = self.fetch_statement(file_path, sheet_name, period_column, statement_type)
            ...
            logger.info(f"Created {statement_type} graph with {len(fsg.graph.nodes)} nodes from Excel file")
            return fsg
