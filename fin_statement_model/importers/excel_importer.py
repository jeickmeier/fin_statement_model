...
class ExcelImporter:
    ...
    def __init__(
        ...
        if self.file_path.suffix not in [".xlsx", ".xls", ".xlsm"]:
            raise ValueError(
                f"Invalid file format. Expected Excel file, got: {self.file_path.suffix}"
            )
    ...
    def standardize_dataframe(
        self, df: pd.DataFrame, structure: Dict[str, Union[str, List[int]]]
    ) -> pd.DataFrame:
        """
        ...
        if structure["orientation"] == "horizontal":
            ...
            header_names = [
                "_".join(str(x) for x in col).strip("_") for col in zip(*headers.values)
            ]
