...
class FMPAdapter(APIDataSourceAdapter):
    ...
    def authenticate(self) -> bool:
        ...
        if not self.api_key:
            ...
            if not self.api_key:
                raise ValueError("No API key provided. Please provide an API key or set the FMP_API_KEY environment variable.")
    ...
    def fetch_statement(self, identifier: str, period_type: str = 'FY', 
                      limit: int = 50, statement_type: str = 'income_statement') -> Dict:
        """
        ...
        if statement_type not in valid_statement_types:
            raise ValueError(f"Invalid statement type: {statement_type}. Valid types are: {valid_statement_types}")
        ...
        try:
            ...
            logger.debug(f"Successfully fetched {len(data)} periods of {statement_type} data for {identifier}")
            return data
    ...
    def create_statement_graph(self, identifier: str, period_type: str = 'FY', 
                             limit: int = 50, statement_type: str = 'income_statement') -> FinancialStatementGraph:
        """
        ...
        try:
            ...
            logger.info(f"Created {statement_type} graph for {identifier} with {len(fsg.graph.nodes)} nodes")
            return fsg
    ...
    def _add_common_calculations(self, fsg: FinancialStatementGraph, statement_type: str) -> None:
        """
        ...
        try:
            if statement_type == 'income_statement':
                ...
                self._try_add_calculation(fsg, "gross_profit", ["revenue", "cost_of_goods_sold"], "subtraction")
                self._try_add_calculation(fsg, "operating_expenses", 
                                         ["research_and_development_expenses", 
                                          "general_and_administrative_expenses",
                                          "selling_and_marketing_expenses",
                                          "selling_general_and_administrative_expenses"], 
                                         "addition")
                self._try_add_calculation(fsg, "operating_income", ["gross_profit", "operating_expenses"], "subtraction")
                self._try_add_calculation(fsg, "ebit", ["operating_income"], "addition")
                self._try_add_calculation(fsg, "ebitda", ["ebit", "depreciation_and_amortization"], "addition")
                self._try_add_calculation(fsg, "net_income_margin", ["net_income", "revenue"], "division")
                
            elif statement_type == 'balance_sheet':
                # Add balance sheet calculations if nodes exist
                self._try_add_calculation(fsg, "cash_and_short_term_investments", 
                                         ["cash_and_cash_equivalents", "short_term_investments"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_current_assets", 
                                         ["cash_and_cash_equivalents", "short_term_investments", 
                                          "net_receivables", "accounts_receivables", "other_receivables", 
                                          "inventory", "prepaids", "other_current_assets"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_non_current_assets", 
                                         ["property_plant_equipment_net", "goodwill", "intangible_assets", 
                                          "goodwill_and_intangible_assets", "long_term_investments", 
                                          "tax_assets", "other_non_current_assets"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_assets", 
                                         ["total_current_assets", "total_non_current_assets"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_current_liabilities", 
                                         ["account_payables", "short_term_debt", "deferred_revenue", 
                                          "tax_payables", "accrued_expenses", "other_current_liabilities"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_non_current_liabilities", 
                                         ["long_term_debt", "deferred_revenue_non_current", 
                                          "deferred_tax_liabilities_non_current", "other_non_current_liabilities"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_liabilities", 
                                         ["total_current_liabilities", "total_non_current_liabilities"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_equity", 
                                         ["common_stock", "retained_earnings", "treasury_stock", 
                                          "additional_paid_in_capital", "accumulated_other_comprehensive_income_loss"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_liabilities_and_equity", 
                                         ["total_liabilities", "total_equity"], 
                                         "addition")
                
            elif statement_type == 'cash_flow':
                # Add cash flow calculations if nodes exist
                self._try_add_calculation(fsg, "free_cash_flow", 
                                         ["operating_cash_flow", "capital_expenditure"], 
                                         "addition")
                                         
                self._try_add_calculation(fsg, "total_cash_flow", 
    ...
    def _try_add_calculation(self, fsg: FinancialStatementGraph, name: str, 
                           inputs: List[str], operation_type: str) -> bool:
        """
