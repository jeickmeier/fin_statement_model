...
class ImportManager:
    ...
    def __init__(self):
        ...
        logger.info(f"ImportManager initialized with {len(self.adapter_factory.list_adapters())} available adapters")
    ...
    def import_from_api(self, source: str, identifier: str, 
                      period_type: str = 'FY', limit: int = 10, 
                      statement_type: str = 'income_statement', **kwargs) -> FinancialStatementGraph:
        """
        ...
        try:
            ...
            logger.info(f"Imported {statement_type} data for {identifier} from {source} API")
            return graph
    ...
    def import_from_excel(self, file_path: str, sheet_name: str, 
                        period_column: str, statement_type: str = 'income_statement',
                        mapping_config: Optional[Dict] = None) -> FinancialStatementGraph:
        """
    ...
    def import_from_csv(self, file_path: str, date_column: str, 
                      value_column: str, item_column: str,
    ...
    def import_from_dataframe(self, df: pd.DataFrame, 
        ...
        try:
            ...
            adapter = self.get_adapter('DataFrameAdapter', mapping_config=mapping_config)
            ...
            logger.info(f"Imported {statement_type} data from DataFrame with {len(df)} rows")
            return graph
    ...
    def register_custom_adapter(self, adapter_type: str, adapter_class: Type[DataSourceAdapter]) -> None:
        """
