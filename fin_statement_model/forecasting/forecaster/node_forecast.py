...
from fin_statement_model.forecasting.methods import BaseForecastMethod
...
def _calc_bad_value(**kwargs: Any) -> float:
    ...
    """
    return float(
        kwargs.get("bad_forecast_value", cfg("forecasting.default_bad_forecast_value"))
    )
...
def _forecast_node_mutating(
    ...
    **kwargs: Any,
    ...
    Example:
        >>> _forecast_node_mutating(
        ...     node, ["2022"], ["2023"], forecast_config, bad_forecast_value=0.0
        ... )
    """
    ...
    if not hasattr(node, "values") or not isinstance(node.values, dict):
        ...
        raise ForecastNodeError(
            f"Node {node.name} lacks a 'values' dictionary.", node_id=node.name
        )

    bad_value = _calc_bad_value(**kwargs)
    allow_neg = kwargs.get(
        "allow_negative_forecasts", cfg("forecasting.allow_negative_forecasts")
    )
    ...
    for period in forecast_periods:
        try:
            ...
            logger.error(
                "Error forecasting %s@%s: %s", node.name, period, exc, exc_info=True
            )
            val = bad_value
...
def forecast_node_non_mutating(
    ...
    **kwargs: Any,
    ...
    historical_periods = (
        [base_period]
        if base_period
        else PeriodManager.infer_historical_periods(fsg, forecast_periods)
    )
    ...
    if forecast_config is None:
        ...
        forecast_config = {
            ...
            "config": (
                cfg("forecasting.default_growth_rate")
    ...
    try:
        ...
        logger.error(
            "Failed to create temp forecast node for %s: %s",
            node_name,
            exc,
            exc_info=True,
        )
    ...
    for period in forecast_periods:
        try:
            ...
            logger.warning(
                "Error calculating forecast for %s@%s: %s", node_name, period, exc
            )
            val = bad_value
